...
export const Sidebar: React.FC<SidebarProps> = ({ alertCount }) => {
  ...
  return (
    <aside className="w-64 bg-surface-900 border-r border-slate-800 flex flex-col shrink-0 min-h-screen">
      {/* Brand Header */}
      <div className="p-5 border-b border-slate-800/80 flex items-center justify-between">
        <NavLink to="/" className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-brand-600 to-indigo-500 flex items-center justify-center shadow-glow-brand">
          ...
          return (
              ...
              className={({ isActive }) =>
                `flex items-center justify-between px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all ${
                  isActive
                    ...
                    : item.highlight
                    ? 'text-brand-300 hover:bg-brand-950/40 hover:text-brand-200 border border-brand-800/40'
            ...
            >
              ...
              {item.highlight && !item.badge && (
                <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-brand-500/20 text-brand-300 uppercase">
                  AI
                </span>
              )}
            </NavLink>
      ...
      <div className="p-4 m-3 rounded-xl bg-surface-950/80 border border-slate-800/80 text-xs">
