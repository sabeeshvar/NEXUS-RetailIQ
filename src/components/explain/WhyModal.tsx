...
import { X, CheckCircle2, AlertTriangle, Calculator, Database, ShieldAlert } from 'lucide-react';
...
export const WhyModal: React.FC<WhyModalProps> = ({ explanation, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div 
        className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto neu-card-lg p-6 text-slate-100 shadow-[16px_16px_40px_#04060c,-16px_-16px_40px_#182338]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-start justify-between pb-4 border-b border-slate-800/80">
          <div className="flex items-center gap-3">
            <div className="p-3 neu-sunken rounded-2xl text-brand-400">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 text-[10px] font-medium neu-sunken-sm text-slate-300 rounded-full border border-slate-700/40">Zero Hallucination</span>
              </div>
          ...
          <button
            onClick={onClose}
            className="neu-btn p-2 rounded-xl text-slate-400 hover:text-white cursor-pointer"
        ...
        <div className="mt-5 space-y-5 text-sm">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {explanation.dataUsed.map((item, idx) => (
                <div key={idx} className="p-3.5 neu-sunken rounded-xl flex flex-col justify-between">
          ...
          <div>
            ...
            <div className="space-y-2 neu-sunken p-4 rounded-xl">
              {explanation.calculationSteps.map((step, idx) => (
                <div key={idx} className="pb-3 last:pb-0 border-b last:border-0 border-slate-800/80">
                  ...
                  <div className="mt-1.5 flex flex-wrap items-center justify-between gap-2 text-xs font-mono">
                    <span className="text-slate-400 neu-sunken-sm px-2 py-1 rounded-lg">{step.formula}</span>
                    <span className="text-brand-300 font-bold neu-card-sm px-2.5 py-1 rounded-lg">
                      = {step.result}
                    </span>
                  </div>
                </div>
          ...
          <div className="p-3.5 neu-card-warning rounded-xl flex items-start gap-3">
          ...
          <div className="p-4 neu-card-success rounded-xl">
        ...
        <div className="mt-6 pt-4 border-t border-slate-800/80 flex justify-end">
          <button
            onClick={onClose}
            className="neu-btn px-4 py-2 text-slate-200 text-sm font-medium rounded-xl cursor-pointer"
          >
            Close Explanation
          </button>
        </div>
